...
sealed class Attunement(layers: LayerSurface, name: String) : Layer<Attunement>(layers, name) {
    private class Attune<L : AttunementItem<L>>(layers: LayerSurface, item: L)
        : Attunement(layers, "Attune ${item.name}") {
        init {
            put(Attunement::class.java, add(item))
        }
    }
    ...
    private class Detune<L : AttunementItem<L>>(layers: LayerSurface, item: L)
        ...
        init {
            put(Attunement::class.java, remove(item))
        }
    }

    companion object {
        fun <L : AttunementItem<L>> attune(item: L): (LayerSurface) -> Attunement {
            fun apply(layers: LayerSurface): Attunement = Attune(layers, item)
            return ::apply
        }

        fun <L : AttunementItem<L>> detune(item: L): (LayerSurface) -> Attunement {
