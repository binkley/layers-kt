...
open class XMutableLayers<K : Any, V : Any, M : XMutableLayer<K, V, M>>(
...
) : AbstractMap<K, V>(), XLayers<K, V, M>, XRules<K, V> {
    init {
    ...
    /** Gets the current-most, mutable layer (the override adds mutation). */
     ...
     * a new default layer.
     *
     * @todo A better way to copy the current stack & mutate on the side
     */
    override fun whatIf(
        name: String,
